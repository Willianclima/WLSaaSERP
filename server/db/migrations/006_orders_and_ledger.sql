-- =========================================================================
-- MIGRATION: 006_orders_and_ledger.sql
-- DESCRIPTION: Orders, Items (with immutable snapshots), Payments,
--              FSM Transitions, and Inventory Movements (Stock Ledger).
-- =========================================================================
...
-- 2. ORDER ITEMS (WITH IMMUTABLE PRODUCT SNAPSHOT)
CREATE TABLE IF NOT EXISTS order_items (
    ...
    -- Immutable product snapshot: preserves SKU, name, pricing, bath/metal specs
...
-- 5. INVENTORY MOVEMENTS (STOCK LEDGER)
CREATE TABLE IF NOT EXISTS inventory_movements (
    id VARCHAR(64) PRIMARY KEY,
    organization_id VARCHAR(64) NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    product_id VARCHAR(64) NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    location_id VARCHAR(64) NOT NULL REFERENCES inventory_locations(id) ON DELETE CASCADE,
    type VARCHAR(50) NOT NULL, -- PURCHASE, SALE, RETURN, LOSS, ADJUSTMENT_UP, ADJUSTMENT_DOWN, CONSIGNMENT_OUT, CONSIGNMENT_RETURN, PRODUCTION_IN
    quantity_change INT NOT NULL,
    physical_balance_after INT NOT NULL,
    consigned_balance_after INT NOT NULL DEFAULT 0,
    reference_type VARCHAR(100), -- ORDER, CONSIGNMENT, PRODUCTION, ADJUSTMENT
...
CREATE INDEX IF NOT EXISTS idx_inv_movements_org_prod ON inventory_movements (organization_id, product_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_inv_movements_org_ref ON inventory_movements (organization_id, reference_type, reference_id);
