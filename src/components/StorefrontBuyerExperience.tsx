...
export const StorefrontBuyerExperience: React.FC<StorefrontBuyerExperienceProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-stone-900 flex flex-col font-sans selection:bg-amber-100 selection:text-amber-900">
      ...
      <header className="sticky top-0 z-30 bg-[#FAF9F6]/95 backdrop-blur-md border-b border-stone-200/80 transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-3 sm:gap-6 h-16 sm:h-20">
            {/* Left: Brand Identity & Optional Back */}
            <div className="flex items-center gap-3 shrink-0">
              {onNavigateToHome && (
                <button
                  onClick={onNavigateToHome}
                  className="p-1.5 -ml-1.5 rounded-full text-stone-500 hover:text-stone-900 hover:bg-stone-200/60 transition-colors cursor-pointer"
              ...
              <div
                onClick={onNavigateToHome}
                className={`flex items-baseline gap-2.5 ${onNavigateToHome ? "cursor-pointer group" : ""}`}
                title={onNavigateToHome ? "Ir para o início" : undefined}
              >
                {branding?.logoType === "IMAGE" && branding.logoUrl ? (
                  ...
                  <h1 className="text-2xl sm:text-3xl font-serif italic font-bold tracking-tight text-stone-900 group-hover:text-amber-800 transition-colors">
                    {branding?.logoText || tenant?.name || "Lumina"}
                  </h1>
                )}
                <span className="text-[9px] font-bold tracking-[0.25em] uppercase font-sans text-amber-800 hidden lg:inline-block">
            ...
            <div className="flex-1 max-w-xs sm:max-w-md relative">
              ...
              <input
                ...
                className="w-full bg-stone-100/90 hover:bg-stone-100 focus:bg-white border border-stone-200/70 focus:border-stone-400 rounded-full pl-9 pr-8 py-2 text-xs text-stone-900 placeholder-stone-400 focus:outline-none transition-all shadow-2xs"
              />
              {searchTerm && (
                <button
            ...
            {/* Right: Actions (Consultant, Warranty, Share, ERP, Cart) */}
            <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
              {/* Consultant badge / selector (compact) */}
              {resellers.length > 0 && (
                <div className="hidden xl:flex items-center gap-1.5 bg-stone-100/90 border border-stone-200/80 rounded-full px-2.5 py-1 text-[11px] text-stone-700">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="text-stone-400 font-medium">Consultora:</span>
                  <select
                    value={selectedResellerId}
                    onChange={(e) => setSelectedResellerId(e.target.value)}
                    className="bg-transparent text-stone-900 font-semibold cursor-pointer focus:outline-none text-[11px]"
                  >
                    <option value="">Matriz Oficial</option>
                    {resellers.map((r) => (
              ...
              {/* Warranty Lookup Button */}
              <button
                onClick={() => setIsWarrantyLookupOpen(true)}
                className="hidden sm:flex items-center gap-1 px-3 py-2 text-xs font-semibold text-stone-600 hover:text-stone-950 border border-stone-200/80 rounded-full bg-white hover:bg-stone-50 transition-colors shadow-2xs cursor-pointer"
                title="Consultar Certificado de Garantia Digital QR"
              >
                <ShieldCheck className="w-3.5 h-3.5 text-amber-600" />
                <span className="hidden md:inline">Garantia</span>
              </button>
              ...
              <button
                ...
                title="Compartilhar catálogo via WhatsApp e Redes"
              >
              ...
              <button
                ...
                className="hidden lg:flex items-center gap-1 px-3 py-2 bg-stone-100/90 hover:bg-stone-200/80 text-stone-700 border border-stone-200/70 rounded-full text-xs font-bold transition-all cursor-pointer"
              ...
              <button
                ...
                className="relative flex items-center gap-2 px-3.5 sm:px-4 py-2 bg-stone-950 text-white rounded-full text-xs font-bold uppercase tracking-wider hover:bg-stone-800 transition-all shadow-sm cursor-pointer shrink-0"
      ...
      {/* Main Content Area - Direct Focus on Product Grid */}
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 w-full space-y-6">
        {/* Minimalist Category & Filter Toolbar */}
        <div className="bg-white border border-stone-200/80 rounded-2xl p-3 sm:p-4 shadow-xs space-y-3">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            {/* Quick Price & Sort Controls + Count */}
            <div className="flex items-center justify-between lg:justify-end gap-2.5 shrink-0 flex-wrap sm:flex-nowrap">
              {/* Item count badge */}
              <span className="text-[11px] font-bold text-stone-500 uppercase tracking-wider hidden sm:inline-block pr-1">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-stone-50 hover:bg-stone-100 border border-stone-200/70 rounded-xl pl-3 pr-8 py-1.5 text-xs font-semibold text-stone-700 focus:outline-none cursor-pointer shadow-2xs"
                >
