...
export const SidebarNavigation: React.FC<SidebarNavigationProps> = ({
  ...
  return (
    <aside className="w-64 bg-white border-r border-stone-200/90 flex flex-col justify-between shrink-0 min-h-screen select-none font-sans">
      ...
      {/* Footer Profile & Firebase Auth */}
      <div className="p-3 border-t border-stone-100 bg-stone-50/50 space-y-2">
        {/* Firebase Google Auth Badge/Button */}
        {isFirebaseAuthed ? (
          <div className="flex items-center justify-between px-2 py-1 bg-emerald-50 border border-emerald-200/70 rounded-lg text-[11px] text-emerald-800">
            ...
            {onLogout && (
              <button
                onClick={onLogout}
                className="text-[10px] text-stone-500 hover:text-rose-600 underline cursor-pointer"
              >
                Desconectar
              </button>
            )}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-1.5 px-2.5 rounded-lg border border-amber-300/80 bg-white hover:bg-amber-50 text-stone-700 hover:text-amber-950 text-xs font-semibold shadow-2xs transition-all cursor-pointer"
            title="Conectar com conta Google / Firebase"
          >
